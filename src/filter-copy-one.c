...
static const GLchar *copy_one_vs[] = {
...
static const GLchar *copy_one_fs[] = {
	...
	"void main()\n",
	"{\n",
	"    gl_FragColor = texture2D(source, vec2(0.5, 0.5));\n",
	"}"
};

static inline struct copy_one *to_copy_one(struct pipeline_stage *stage)
{
	return (struct copy_one *)stage;
}

static void copy_one_release(struct pipeline_stage *stage)
{
	struct copy_one *copy = to_copy_one(stage);

	glsl_program_free(copy->program);
	free(copy);
}

static void copy_one_render(struct pipeline_stage *stage)
{
	struct copy_one *copy = to_copy_one(stage);
